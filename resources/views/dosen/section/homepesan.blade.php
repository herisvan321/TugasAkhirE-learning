...
<h4 class="w3-text-black">  
		@if($bio->namabelakang == 'kosong')
			<a href="{{url('/homedosen/view/pesan/'.$t->id_pesan)}}" title="">{{$bio->namadepan}}</a>
		@else
			<a href="{{url('/homedosen/view/pesan/'.$t->id_pesan)}}" title="">{{$bio->namadepan}} {{$bio->namabelakang}}</a>
		@endif
		<br>
		<i class="w3-text-gray">{{ substr($t->pesan_terakhir, 0,35)}}</i>
		<span class="w3-right">
	@if($t->jumlah1 > 0)
	<span class="bg-danger w3-padding w3-circle w3-text-white">{{$t->jumlah1}} </span>
	@endif
</span>
		</h4>
